...
//		response.getWriter().append("Served at: ").append(request.getContextPath());
		...
		// 글목록 출력
		response.setContentType("text/html;charset=utf-8");  //웹브라우저의 컨텐츠타입 html 형식으로 지정 (기본은 텍스트)
		System.out.println("Hello Servlet !!!");  //콘솔에 정보가 그려짐
		PrintWriter out = response.getWriter();   //stream 하나 생성
		out.println("<b>Hello, Servlet</b>");  //사용자화면에 정보가 출력됨(여기서는 웹브라우저)
